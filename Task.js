...
export default function Task({ task, children }) {
  ...
  const { title, desc, labels, priority,assignees} = task;
  ...
  return (
    ...
    >
      <Title title={title} controls={children} priority={priority} assignees={assignees}/>
...
function Title({ title, controls, priority }) {
  return (
